...
- Select **Settings** and enter the host password to edit the menu, update the event date, or manage guest accounts. Select an account for the current event to allow that family to sign in; selections reset automatically after the event date passes. After the host signs in, the button is labeled **Host tools**.
